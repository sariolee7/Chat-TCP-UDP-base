...
public class WavUtility
{
	...
	public static AudioClip ToAudioClip (byte[] fileBytes, int offsetSamples = 0, string name = "wav")
	{
		...
		// AudioClip.Create expects samples per channel, not total float count
		int samplesPerChannel = data.Length / channels;
		AudioClip audioClip = AudioClip.Create (name, samplesPerChannel, (int)channels, sampleRate, false);
	...
	public static byte[] FromAudioClip (AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
	{
		...
		// total file size = 44 bytes for header format and audioClip.samples * channels * factor due to float to Int16 / sbyte conversion
	...
	private static int WriteFileData (ref MemoryStream stream, AudioClip audioClip, UInt16 bitDepth)
	{
		...
		float maxVal = 0f;
		for (int j = 0; j < Mathf.Min(data.Length, 1000); j++) {
			if (Mathf.Abs(data[j]) > maxVal) maxVal = Mathf.Abs(data[j]);
		}
